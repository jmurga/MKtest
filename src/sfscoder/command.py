        ...
        * Parameters:
        ...
        if(isinstance(self,SFSCommand)):
            if self.sfs_code_loc == '':
                ...
                print('set location with: <SFSCommand>.sfs_code_loc =', end=' ')
        ...
        if(isinstance(self, MSCommand)):
            if self.ms_loc == '':
                ...
                print('set location with: <MSCommand>.ms_loc =', end=' ')
                print('/path/to/ms')
                exit()
    ...
    * Attributes:
    ...
    def convert_sfs_ms(self):
        ...
        def get_sizes(s,ti,N,N_b,mig_events,N_m,G_b):
            ...
            for key,value in list(s.__dict__.items()):
        ...
        if self.com_string == '':
            print('No command string to parse', file=sys.stderr)
            exit(-1)
    ...
    def genomic(self,basedir=os.path.join(os.path.dirname(__file__),'req'),
        ...
        * Parameters:
          ...
          * *basedir=os.path.join(os.path.dirname(__file__),'req')*
        ...
        if model != 'snm':
            ...
            print('supported models include snm (standard neutral)', end=' ')
    ...
    def snm(self,N=500,nsim=10,t=0.001,rho=0.001,nsam=[10],recombfile='',
        ...
        if len(nsam) > 1:
            print("There is only one population in the standard neutral model!", file=sys.stderr)
            print("Using first population size given", file=sys.stderr)
    ...
    def build_genomic(self,basedir=os.path.join(os.path.dirname(__file__),'req'),
                      indir=os.path.join(os.getcwd(),'data'),
    ...
    def three_pop(self,add_on=False,nsim=1,N=10000,non_coding=False,
        ...
        # these are the pre-publication parameters from Dadi (provided by Ryan Hernandez)
            ...
            print("supported models: 'gutenkunst', 'gravel', 'tennessen', and ''", end=' ')
