    ...
    * Attributes:
    ...
    def calc_S(self,multi_skip=True,loci=[],pops=[0],input_log='',start=-1,stop=-1):
        ...
        * Parameters:
        ...
        if len(loci) == 0:
            loci = list(range(0,len(self.command.L)))
        ...
        * Parameters:
        ...
        for pop in pops:
            if S[pop] is None or watt[pop] is None or pi[pop] is None:
                print("Population", pop, "has no samples", file=sys.stderr)
                continue
    ...
    def check_L(self,arr):
        ...
        test_arr = list(range(0, len(self.command.L)))    

        for thing in arr:
            if thing not in test_arr:
                print('Warning:', thing,  'not in self.command.L', file=sys.stderr)
                print('Removing from list', file=sys.stderr)
                continue
        ...
        * Parameters:
        ...
        for pop in pops: 
            ...
            if self.command.n[pop] > 0:
                ...
                print("Warning: No samples in population", pop, file=sys.stderr)
            pis[pop] = pi
        return pis
        ...
        * Parameters:
        ...
        for pop in pops:
            if pop not in self.command.n:
                print("No such pop,", pop, "!", file=sys.stderr)
            else:
                new_pops.append(pop)
    ...
    def get_loci_within_coordinates(self,regstart=-1,start=-1,stop=-1):
        ...
        if((start < 0 or stop < 0) or stop < start):
            ...
            print("Warning: Must declare a positive start and stop!", file=sys.stderr)
            print("Must have start > stop!", file=sys.stderr)
            print("loci will be set to empty set", file=sys.stderr)
            return [] 
        ...
        if method == 'SKAT':
            ...
            if min_freq == -1:
                if 1./self.command.n[causal_pop] > 0.03:
                    print("Sample size is ", self.command.n[causal_pop], file=sys.stderr)
                    print("Resetting min_freq to 1/", self.command.n[causal_pop], file=sys.stderr) 
           ...
           return effects 
            ...
            print("Warning: No such method,", mehtod, "!", end=' ', file=sys.stderr)
            print("All effects set to 0!", file=sys.stderr)
            return effects
    ...
    def sim_pheno(self,h_sq=0.01,pops=[0],min_freq=-1,causal_pop=-1,opp=1.,rho=1.,method='SKAT',tau=1.,sig=1.):
        ...
        * *sigma=1* 
        ...
        for pop in pops:
            if pop not in self.command.n:
                print("No samples from population", pop, "!", file=sys.stderr)
            else:
                new_pops.append(pop)
        ...
        if a == 1: 
            print("Genetic variance of pop", pop, " is 0!", file=sys.stderr)
            print("All variance in phenotype will come from environment", file=sys.stderr)
    ...
    * Attributes:
    ...
    def get_sims(self,iter=[]):
        ...
        try: 
            ...
            print('Error: cannot open file', self.file, 'for reading!', file=sys.stderr) 
            exit(-1)
        ...
        for line in f:
            ...
            if line_num == 0:
                ...
                if not iter:
                    iter = list(range(0,sim.command.nsim))
...
class myTests(unittest.TestCase):
    ...
    def testGetSims(self):
        self.assertTrue(self.data.get_sims())
    ...
    def testS(self):
        ...
        for sim in self.data.sims:
            S_test =  sim.calc_S(pops = list(range(0,sim.command.n_pops)))
    ...
    def testZnS(self):
        ...
        self.assertTrue(str(ZnS_test[0]) == ZnS[0])
        self.assertTrue(str(ZnS_test[1]) == ZnS[1])
    ...
    def testWatt(self):
        ...
        self.assertTrue(str(watt_test[0]) == watt[0])
        self.assertTrue(str(watt_test[1]) == watt[1])
    ...
    def testTheta_H(self):
        ...
        self.assertTrue(str(H_test[0]) == H[0])
        self.assertTrue(str(H_test[1]) == H[1])
    ...
    def testTajD(self):
        ...
        self.assertTrue(str(D_test[0]) == D[0])
        self.assertTrue(str(D_test[1]) == D[1])
    ...
    def testHaplo(self):
        ...
        for sim in self.data.sims: 
            self.assertTrue(list(map(int,np.array(h))) == list(map(int,np.array(sim.haplo[0][0]))))
    ...
    def testGetsfs(self):
        ...
        self.assertTrue(str(sfs_test[0][0]) == s[0])
